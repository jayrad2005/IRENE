  ...
  start: async (
    ...
    switch (inputCMD) {
      ...
      case "setmod":
        if (!text && !m.quoted) {
          await doReact("❌");
          return m.reply(`Tag a user to make *mod*!`);
        }
        ...
        if (!isCreator && !isintegrated && isUsermod) {
          await doReact("❌");
          return m.reply(
            "SCRAM!! only my *Owner* can use this command ! *Added Mods* does not have this permission."
          );
        }
      ...
      case "removemod":
        ...
        if (!isCreator && !isintegrated && isUsermod) {
          await doReact("❌");
          return m.reply(
            "SIGHS... only my *Owner* can use this command ! *Added Mods* does not have this permission."
          );
        }
        if (!userId) return m.reply("Please mention a valid user to ban!");
      ...
      case "banuser":
        ...
        if (checkUserModStatus == true || globalOwner.includes(userNum)) {
          await doReact("❌");
          return m.reply(`I can't ban an *Owner* or *Mod* damn it !`);
        }
        if (chechBanStatus) {
          await doReact("✅");
          return Atlas.sendMessage(
      ...
      case "pmchatbot":
        ...
        if (args[0] === "on") {
          if (pmChatBotStatus) {
            ...
            await m.reply(
              `*PM Chatbot* has been *Enabled* Successfully ! \n\nLORD HIGH BOT will reply to all chats in PM !`
            );
          }
        } else if (args[0] === "off") {
          if (!pmChatBotStatus) {
      ...
      case "mode":
        ...
        if (args[0] == "self") {
          ...
          if (chechbotMode == "private") {
            await doReact("❌");
            return m.reply(
              `LORD HIGH BOT is already in *Private* mode !\n\nOnly bot *Owners / Mods* can use bot.`
            );
          } else {
            await doReact("🧩");
            await setBotMode("private");
            await m.reply(`LORD HIGH BOT has been set to *Private* mode Successfully !`);
          }
        } else if (args[0] == "public") {
          if (chechbotMode == "public") {
            await doReact("❌");
            return m.reply(
              `LORD HIGH BOT is already in *Public* mode !\n\nAnyone can use bot.`
            );
          } else {
            await doReact("🧩");
            await setBotMode("public");
            await m.reply(`LORD HIGH BOT has been set to *Public* mode Successfully !`);
          }
        } else {
          await doReact("❌");
          return m.reply(
