  ...
  start: async (
    ...
    switch (inputCMD) {
      ...
      case "setgcname":
        if (!isAdmin) {
          await doReact("❌");
          return m.reply(`*IDIOT* you must be *Admin* in order to use this Command!`);
        }
        if (!isBotAdmin) {
          await doReact("❌");
          return m.reply(`*LORD HIGH BOT* must be *Admin* in order to use this Command!`);
        }
        if (!text) {
          await doReact("❔");
          return m.reply(
      ...
      case "del":
        if (!isAdmin) {
          await doReact("❌");
          return m.reply(`*WEREY* you must be *Admin* in order to use this Command!`);
        }
        if (!m.quoted) {
          await doReact("❔");
        ...
        if (!isBotAdmin) {
          if (!m.quoted.sender.includes(botNumber)) {
            await doReact("❌");
            return m.reply(
              `Listen Human, Without *Admin* permission, I can only delete my own messages !`
            );
          }
          key = {
            remoteJid: m.from,
          ...
          if (!isAdmin) {
            await doReact("❌");
            return m.reply(
              `SCUMBAG, only *Admins* can delete other's messages !`
            );
          }
          key = {
            remoteJid: m.from,
      ...
      case "demote":
        if (!isAdmin) {
          await doReact("❌");
          return m.reply(`*SIGHS,* you must be *Admin* in order to use this Command!`);
        }
        if (!isBotAdmin) {
          await doReact("❌");
          return m.reply(`*LORD HIGH BOT* must be *Admin* in order to use this Command!`);
        ...
        if (quotedsender.includes(botNumber)) {
          await doReact("❌");
          return m.reply(`Nice try shithead, I can't demote myself !`);
        }

        if (!text && !m.quoted) {
          await doReact("❔");
          return m.reply(`TAG a user to *Demote*!`);
        } else if (m.quoted) {
        ...
        if (!groupAdmin.includes(userId)) {
          return Atlas.sendMessage(
            m.from,
            {
              ...
              } The HUMAN is not an *Admin* of this group!`,
            ...
            (res) =>
              Atlas.sendMessage(
                m.from,
                {
                  text: `Sorry @${
                    mentionedUser.split("@")[0]
                  } Dog Brain, you have been *Demoted* by @${
      ...
      case "gc":
        if (!isAdmin) {
          await doReact("❌");
          return m.reply(`*Idiot* must be *Admin* in order to use this Command!`);
        }
        if (!isBotAdmin) {
          await doReact("❌");
          return m.reply(`*LORD HIGH BOT* must be *Admin* in order to use this Command!`);
      ...
      case "promote":
        if (!isAdmin) {
          await doReact("❌");
          return m.reply(`** mMARAFACKAust be *Admin* in order to use this Command!`);
        }
        if (!isBotAdmin) {
          await doReact("❌");
          return m.reply(`*LORD HIGH BOT* must be *Admin* in order to use this Command!`);
        ...
        if (groupAdmin.includes(userId)) {
          return Atlas.sendMessage(
            m.from,
            {
              text: `@${
                mentionedUser.split("@")[0]
              } This HUMAN is already an *Admin* of this group!`,
              mentions: [mentionedUser],
            },
            { quoted: m }
          );
              ...
              Atlas.sendMessage(
                m.from,
                {
                  text: `Congratulations  @${
                    mentionedUser.split("@")[0]
                  } HUMAN 🥳, you have been *Promoted* by @${
                    messageSender.split("@")[0]
                  } !`,
                  mentions: [mentionedUser, messageSender],
                },
      ...
      case "remove":
        if (!isAdmin) {
          await doReact("❌");
          return m.reply(`*Dumbfuck* must be *Admin* in order to use this Command!`);
        }
        if (!isBotAdmin) {
          await doReact("❌");
          return m.reply(`*LORD HIGH BOT* must be *Admin* in order to use this Command!`);
          ...
          return Atlas.sendMessage(
            m.from,
            {
              text: `*Command Rejected !* @${
                mentionedUser.split("@")[0]
              } Why're you so dumb? Nigga is an *Admin* of this group so you are not allowed to remove him !`,
              mentions: [mentionedUser],
            },
            { quoted: m }
          );
          ...
          (res) =>
            Atlas.sendMessage(
              m.from,
              {
                text: `@${
                  mentionedUser.split("@")[0]
                } has been *Kicked* Successfully from *${metadata.subject}*`,
                mentions: [mentionedUser],
              },
              { quoted: m }
            )
      ...
      case "revoke":
        ...
        if (m.from == "12036304083875395@g.us") {
          await doReact("❌");
          return m.reply(
            "This command ain't allowed in *this Group* !\n\nYou are not allowed to change this group link !"
          );
        }
        await doReact("💫");
      ...
      case "tagall":
        if (!isAdmin) {
          await doReact("❌");
          return m.reply(`*FOOL* you must be *Admin* in order to use this Command!`);
        }
        if (!isBotAdmin) {
          await doReact("❌");
          return m.reply(`*LORD HIGH BOT* must be *Admin* in order to use this Command!`);
...
│\n`;
      ...
      case "chatbotgc":
        ...
        if (args[0] == "on") {
          ...
          await m.reply(
            `*Group Chatbot* has been *Enabled* Successfully ! \n\nLORD HIGH BOT will not reply to messages where bot is mentioned!`
          );
        } else if (args[0] == "off") {
          if (!chatbotGCStatus) {
            await doReact("❌");
      ...
      case "antilink":
        if (!isAdmin) {
          await doReact("❌");
          return m.reply(`*GOAT* must be *Admin* in order to use this Command!`);
        }
        if (!isBotAdmin) {
          await doReact("❌");
          return m.reply(`*LORD HIGH BOT* must be *Admin* in order to use this Command!`);
        ...
        if (args[0] == "on") {
          ...
          await m.reply(
            `*Antilink* has been *Enabled* Successfully ! \n\nLORD HIGH BOT will remove all links sent by WORTHLESS HUMANS from messages!`
          );
        } else if (args[0] == "off") {
          if (!antilinkStatus) {
            await doReact("❌");
